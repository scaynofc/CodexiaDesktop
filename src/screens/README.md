...
(Phase 4), `Timeline.tsx` (Phase 5), `Providers.tsx` (Phase 6), and
`Runtime.tsx` (Phase 7) are built. The rest (Approvals, Memory, Logs,
...
`connectionStore`); `metricsStore` (Phase 6) and `runtimeStore` (Phase 7)
instead expose a `fetch*()` action the screen calls on mount/refresh,
since CodexiaCore has no change-notification event for either - see
`docs/adr/009-provider-center-metrics-snapshot.md` and
`docs/adr/010-runtime-center-ollama-proxy.md`.
